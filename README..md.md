...
- Case-study flag for posts that document a specific project
...
| Method | Endpoint                  | Description                                              | Auth |
| ------ | -------------------------- | ---------------------------------------------------------- | ---- |
| GET    | `/api/posts`               | Get published posts (search, category, tag, isCaseStudy) | ❌   |
...
| POST   | `/api/projects/upload-video`   | Upload a project preview video to Cloudinary       | ✅   |
...
skills section, and a closing call to action.
...
Blog index with search (synced to the URL via `useSearchParams`),
pagination, loading skeletons, empty state, and error state.
...
Project editor: title, description, tech-stack chips, image and video
upload, live/GitHub URLs, and a featured toggle.
